...
async function bootstrap(): Promise<Server | void> {
  ...
  app.enableCors({
    ...
    exposedHeaders: ['Content-Length', 'Date', 'X-RateLimit-Limit', 'X-RateLimit-Remaining', 'X-RateLimit-Reset'],
...
exports.handler = async (event: any, context: Context) => {
  try {
    ...
    event.path = (event.path.includes('swagger-ui') || event.path.includes('swagger'))
